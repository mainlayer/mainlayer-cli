...
- [ ] **VEND-01**: Vendor can create a new resource with slug, description, price (USDC), fee model, and type (`mainlayer resource create`)
- [ ] **VEND-02**: Vendor can list all their resources (`mainlayer resource list`)
- [ ] **VEND-03**: Vendor can get details of a single resource (`mainlayer resource get`)
- [ ] **VEND-04**: Vendor can update a resource's fields (`mainlayer resource update`)
- [ ] **VEND-05**: Vendor can delete a resource (`mainlayer resource delete`)
- [ ] **VEND-06**: Vendor can create a pricing plan for a resource (`mainlayer resource plan create`)
- [ ] **VEND-07**: Vendor can list, update, and delete pricing plans for a resource (`mainlayer resource plan list/update/delete`)
- [ ] **VEND-08**: Vendor can set, get, and delete per-wallet quota limits for a resource (`mainlayer resource quota set/get/delete`)
- [ ] **VEND-09**: Vendor can create, list, and delete discount coupons for a resource (`mainlayer coupon create/list/delete`)
- [ ] **VEND-10**: Vendor can update the webhook URL, view delivery logs, retry failed webhooks, and rotate the webhook secret (`mainlayer webhook`)

### Vendor — Analytics

- [ ] **ANAL-01**: Vendor can view earnings summary (total volume, fees, net) with optional resource and date filters (`mainlayer earnings`)
- [ ] **ANAL-02**: Vendor can view aggregated metrics (active resources, payment counts by status) (`mainlayer metrics`)

### Buyer — Discovery & Purchase

- [ ] **BUYR-01**: Buyer can search and list discoverable resources (`mainlayer discover`)
- [ ] **BUYR-02**: Buyer can purchase a resource using the embedded Solana wallet via X402 payment flow (`mainlayer buy`)
- [ ] **BUYR-03**: Buyer can view their active entitlements (access rights) (`mainlayer entitlements`)

### Buyer — Subscriptions

- [ ] **SUBS-01**: Buyer can approve auto-renewal for a subscription resource (`mainlayer subscribe approve`)
- [ ] **SUBS-02**: Buyer can pause a subscription (`mainlayer subscribe pause`)
- [ ] **SUBS-03**: Buyer can resume a paused subscription (`mainlayer subscribe resume`)
- [ ] **SUBS-04**: Buyer can list all their subscriptions (`mainlayer subscribe list`)
- [ ] **SUBS-05**: Buyer can get details of a single subscription (`mainlayer subscribe get`)

### Buyer — Post-Purchase Operations

- [ ] **POST-01**: Buyer can list their invoices (`mainlayer invoices`)
- [ ] **POST-02**: Buyer can request a refund for a payment (`mainlayer refund request`)
- [ ] **POST-03**: Buyer can create a dispute for a payment (`mainlayer dispute create`)
- [ ] **POST-04**: Buyer can list their disputes (`mainlayer dispute list`)

### Installation & AI Agent Onboarding

- [ ] **INST-01**: npm postinstall script detects AI platform config dirs (Claude Desktop, Claude Code, Cursor, Windsurf, Gemini CLI, VS Code, Zed, Continue) and registers the Mainlayer MCP server; exits 0 even when no platforms are detected
- [ ] **INST-02**: postinstall is fully idempotent — running twice does not duplicate MCP registrations
- [ ] **INST-03**: `mainlayer setup` command re-runs platform detection and MCP registration manually
- [ ] **INST-04**: A `skills.md` file is dropped into each detected AI platform's config directory on install, describing all CLI commands and their usage for AI agents
- [ ] **INST-05**: README and GitHub documentation cover: quickstart for vendors, quickstart for buyers, wallet security model, MCP setup, and full CLI command reference
